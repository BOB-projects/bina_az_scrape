...
class BinaScraper:
    ...
    def extract_item_data(self, node: Dict) -> Dict[str, Any]:
        ...
        item = {
            ...
            # Price information
            "price_value": node.get("price", {}).get("value")
            if node.get("price")
            else None,
    ...
    def load_checkpoint(self) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Loaded checkpoint: {checkpoint['items_count']} items, cursor: {checkpoint.get('cursor', 'None')[:50]}..."
            )
            return checkpoint
    ...
    def save_incremental(self, page_num: int):
        ...
        try:
            filename = (
                f"backup_page{page_num}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    def load_from_checkpoint(self):
        ...
        backup_files = sorted(
            self.output_dir.glob("backup_page*.json"),
        ...
        if backup_files:
            try:
                ...
                self.seen_ids = {
                    item["id"] for item in self.all_items if item.get("id")
                }
                logger.info(
                    f"Loaded {len(self.all_items)} items from backup: {backup_files[0].name}"
                )
            except Exception as e:
    ...
    async def fetch_page(
        self, cursor: Optional[str] = None, attempt: int = 1
    ) -> Optional[Dict]:
            ...
            try:
                ...
                async with self.session.get(url) as response:
                    if response.status == 200:
                        ...
                        logger.warning(
                            f"Cloudflare block detected (403). Launching re-auth..."
                        )
                        # Get new session
                        try:
    ...
    async def scrape_all(self) -> List[Dict]:
        ...
        try:
            while True:
                ...
                if not data or "data" not in data or data["data"] is None:
                    ...
                    logger.error(
                        f"Failed to fetch data or invalid response (failure {consecutive_failures}/{max_consecutive_failures})"
                    )

                    if consecutive_failures >= max_consecutive_failures:
                        logger.error(
                            "Max consecutive failures reached. Stopping scraper."
                        )
                ...
                for edge in edges:
                    ...
                    if node:
                        ...
                        if not self.validate_item(item_data):
                            logger.warning(
                                f"Invalid item skipped: {item_data.get('id', 'unknown')}"
                            )
                            items_skipped += 1
                            continue
        ...
        logger.info(
            f"Duplicates removed:     {len(self.all_items) - len(self.seen_ids):,}"
        )
        ...
        logger.info(
            f"Average per page:       {self.format_time(total_time / page_num if page_num > 0 else 0)}"
        )
        logger.info(
            f"Average per item:       {self.format_time(total_time / len(self.all_items) if self.all_items else 0)}"
        )
        logger.info(f"")
        logger.info(f"FINAL PERFORMANCE")
        logger.info(f"Pages per minute:       {(page_num / total_time * 60):.1f}")
        logger.info(
            f"Items per minute:       {(len(self.all_items) / total_time * 60):.1f}"
        )
    ...
    def save_to_parquet(self, filename: str = None):
        ...
        if filename is None:
            filename = f"bina_sale_{datetime.now().strftime('%Y%m')}.parquet"
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            "total_items": total,
            "with_photos": sum(
                1 for item in self.all_items if item.get("photos_count", 0) > 0
            ),
            "with_mortgage": sum(
                1 for item in self.all_items if item.get("has_mortgage")
            ),
        ...
        # Price statistics
        prices = [
            item["price_value"] for item in self.all_items if item.get("price_value")
        ]
        if prices:
        ...
        # Area statistics
        areas = [
            item["area_value"] for item in self.all_items if item.get("area_value")
        ]
        if areas:
    ...
    def get_data_integrity_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "items_with_missing_price": sum(
                1 for item in self.all_items if not item.get("price_value")
            ),
            "items_with_missing_location": sum(
                1 for item in self.all_items if not item.get("location_name")
            ),
            "items_with_missing_area": sum(
                1 for item in self.all_items if not item.get("area_value")
            ),
            "items_with_photos": sum(
                1 for item in self.all_items if item.get("photos_count", 0) > 0
            ),
            "data_completeness": {
                "id": sum(1 for item in self.all_items if item.get("id"))
                / len(self.all_items)
                * 100,
                "price": sum(1 for item in self.all_items if item.get("price_value"))
                / len(self.all_items)
                * 100,
                "location": sum(
                    1 for item in self.all_items if item.get("location_name")
                )
                / len(self.all_items)
                * 100,
                "area": sum(1 for item in self.all_items if item.get("area_value"))
                / len(self.all_items)
                * 100,
            },
        }
...
async def main():
    ...
    async with BinaScraper() as scraper:
        ...
        if items:
            ...
            # Save to JSON, CSV, XLSX and Parquet formats
            json_file = scraper.save_to_json()
            csv_file = scraper.save_to_csv()
            xlsx_file = scraper.save_to_xlsx()
            parquet_file = scraper.save_to_parquet()
            ...
            logger.info(
                f"  Missing price: {integrity.get('items_with_missing_price', 0)}"
            )
            logger.info(
                f"  Missing location: {integrity.get('items_with_missing_location', 0)}"
            )
            logger.info(
                f"  Missing area: {integrity.get('items_with_missing_area', 0)}"
            )
            ...
            for city, count in sorted(
                stats.get("cities", {}).items(), key=lambda x: x[1], reverse=True
            )[:10]:
